...
docker exec "$ROS_DISTRO"-container /bin/bash -c 'apt update && apt install -y lcov python3-pip libgtest-dev cmake && rosdep update'
...
docker exec "$ROS_DISTRO"-container /bin/bash -c 'source /opt/ros/$ROS_DISTRO/setup.bash && cd /"$ROS_DISTRO"_ws/ && source ./install/setup.bash && lcov --capture --directory . --output-file coverage.info'
docker exec "$ROS_DISTRO"-container /bin/bash -c 'source /opt/ros/$ROS_DISTRO/setup.bash && cd /"$ROS_DISTRO"_ws/ && lcov --remove coverage.info '/usr/*' --output-file coverage.info'
docker exec "$ROS_DISTRO"-container /bin/bash -c 'source /opt/ros/$ROS_DISTRO/setup.bash && cd /"$ROS_DISTRO"_ws/ && lcov --list coverage.info'
docker exec "$ROS_DISTRO"-container /bin/bash -c 'cd /"$ROS_DISTRO"_ws/ && mv coverage.info /shared'