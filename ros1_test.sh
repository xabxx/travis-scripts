...
do
  ...
  docker cp $TRAVIS_BUILD_DIR ${ros1_distro_names_arr[$i]}-container:/"${ros1_distro_names_arr[$i]}"_ws/src/
  ...
  docker exec ${ros1_distro_names_arr[$i]}-container /bin/bash -c 'cd /"$ROS_DISTRO"_ws/ && mv coverage.info /shared'
done
